...
function ExecutiveDashboard() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/reports/executive`)
    ...
    <div className="dashboard-container">
        ...
        <div style={{ width: '100%', height: 400 }}>
          <ResponsiveContainer>
            <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <XAxis dataKey="building" stroke="#94a3b8" tick={{fill: '#94a3b8'}} axisLine={{stroke: '#334155'}} />
              ...
              <Bar dataKey="activity_count" name="Total Activities" fill="url(#colorActivity)" radius={[4, 4, 0, 0]} />
              <Bar dataKey="worker_count" name="Workers Deployed" fill="url(#colorWorker)" radius={[4, 4, 0, 0]} />
              <defs>
      ...
      <div className="card-grid" style={{ marginTop: '30px' }}>
        <div className="stat-card card">
          <div className="stat-value">{data.reduce((acc, curr) => acc + curr.activity_count, 0)}</div>
...
function OperationsCenter() {
  const [batch, setBatch] = useState({ building: 'Block Z', floor: '5', type: 'Cleaning', worker_id: 'W001', time: '' });
  ...
  return (
    <div className="page-container">
      <div className="header-section">
        <h2><Wrench size={28} style={{ marginRight: '10px', color: 'var(--warning)' }} /> Operations Center</h2>
      ...
      <div className="section-block">
        ...
        <div className="data-form grid-form">
          <input placeholder="Building" value={batch.building} onChange={e=>setBatch({...batch, building: e.target.value})}/>
          <input placeholder="Floor" value={batch.floor} onChange={e=>setBatch({...batch, floor: e.target.value})}/>
          <input placeholder="Worker ID" value={batch.worker_id} onChange={e=>setBatch({...batch, worker_id: e.target.value})}/>
          <input type="datetime-local" onChange={e=>setBatch({...batch, time: e.target.value})}/>
          <button onClick={sendBatch} className="action-btn">Dispatch Batch</button>
        </div>
      </div>
      ...
      <div className="section-block" style={{ marginTop: '40px' }}>
        ...
        <div className="data-form">
          ...
          <input placeholder="Building" onChange={e=>setSearch({...search, building: e.target.value})}/>
            ...
            <tbody>
              {schedule.length > 0 ? schedule.map((row, i) => (
                <tr key={i}>
                  ...
                  <td style={{ color: row.chemicals_used === 'Yes' ? 'var(--error)' : 'var(--success)', fontWeight: 'bold' }}>
                    {row.chemicals_used}
                  </td>
                </tr>
              )) : (
                <tr><td colSpan="3" style={{ textAlign: 'center', color: 'var(--text-muted)' }}>No records found</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function AdminConsole() {
  ...
  return (
    <div className="page-container">
      <div className="header-section">
        <h2><Database size={28} style={{ marginRight: '10px', color: 'var(--success)' }} /> Admin Console</h2>
      ...
      <textarea 
        ...
        className="sql-input"
      />
      <button onClick={runQuery} className="primary-btn">Execute Query</button>
      
      {error && <div className="error"><Activity size={16} /> {error}</div>}
        ...
        <div className="result-container">
           {Array.isArray(result.data) ? (
            <div className="table-wrapper">
                <table>
                <thead>
                    <tr>{result.data.length > 0 && Object.keys(result.data[0]).map(k => <th key={k}>{k}</th>)}</tr>
                </thead>
                <tbody>
...
function PersonManager() {
  ...
  return (
    <div className="page-container">
      <div className="header-section">
        <h2><Users size={28} style={{ marginRight: '10px', color: 'var(--secondary)' }} /> Person Management</h2>
    ...
    <Router>
      <div className="app">
        <nav className="sidebar">
          <div className="logo">CMMS <span className="pro-badge">PRO</span></div>
          <NavLink to="/" icon={LayoutDashboard}>Dashboard</NavLink>
          <NavLink to="/ops" icon={Wrench}>Operations</NavLink>
          <NavLink to="/persons" icon={Users}>People</NavLink>
          <NavLink to="/admin" icon={Database}>Admin</NavLink>
        </nav>
