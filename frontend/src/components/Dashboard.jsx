...
const COLORS = ['#A6192E', '#B08E55', '#8C1526', '#5D5D5D', '#A0A0A0', '#D4D4D4'];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError(`Error: ${error.message || "Unknown error"}. ${error.code ? `Code: ${error.code}` : ""}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (error) {
    return (
      <div className="dashboard-container">
        <div className="error-message" style={{ padding: '20px', color: '#ef4444', background: '#fee2e2', borderRadius: '8px' }}>
    ...
    <div className="dashboard-container">
      ...
      <div className="card-grid">
        ...
        <div className="card full-width">
          <h3>Maintenance by Location & Type</h3>
          <div style={{ width: '100%', height: 200 }}>
            <ResponsiveContainer>
              <BarChart data={maintenanceSummary}>
                <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
                <XAxis dataKey="building" stroke="#666" fontSize={12} />
                <YAxis stroke="#666" fontSize={12} />
                <Tooltip contentStyle={{ backgroundColor: '#fff', borderColor: '#ccc', color: '#333' }} />
                <Legend wrapperStyle={{ fontSize: '12px' }} />
                <Bar dataKey="count" fill="#A6192E" name="Tasks" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div style={{ width: '100%', height: 200 }}>
             <ResponsiveContainer>
              <BarChart data={activitiesSummary} layout="vertical">
                ...
                <Tooltip contentStyle={{ backgroundColor: '#fff', borderColor: '#ccc', color: '#333' }} />
                <Bar dataKey="activity_count" fill="#B08E55" name="Count" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
