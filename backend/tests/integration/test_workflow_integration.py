...
class TestSchoolAffiliationWorkflow:
    ...
    def test_create_school_person_affiliation(
        ...
        data = json.loads(response.data)
        assert any(
            a['personal_id'] == sample_person_data['personal_id'] and
            a['department'] == sample_school_data['department']
            for a in data
        )
...
class TestSafetySearchWorkflow:
    ...
    def test_safety_search_with_chemical(self, integration_client, sample_location_data):
