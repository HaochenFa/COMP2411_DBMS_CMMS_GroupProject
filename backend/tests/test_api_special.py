...
class TestSafetySearchEndpoint:
    ...
    def test_safety_search_with_time_period(self, client, mock_get_db_connection):
        ...
        mock_cursor.fetchall.return_value = [
            {'maintenance_id': 1, 'type': 'Cleaning', 'frequency': 'Daily',
             'location_id': 1, 'active_chemical': False, 'building': 'Block A',
             'room': '101', 'floor': '1', 'campus': 'Main',
             'scheduled_time': '2024-01-15T10:00:00', 'end_time': '2024-01-15T12:00:00',
             'company_name': None}
        ]
...
class TestBuildingSupervisionEndpoints:
    ...
    def test_get_building_supervisions(self, client, mock_get_db_connection):
        ...
        mock_conn, mock_cursor = mock_get_db_connection
        mock_cursor.fetchall.return_value = [
            {'supervision_id': 1, 'personal_id': 'P001', 'building': 'Block A',
             'assigned_date': '2024-01-01', 'manager_name': 'John Manager'}
        ]
    ...
    def test_create_building_supervision_missing_fields(self, client, mock_get_db_connection):
    ...
    def test_delete_building_supervision_not_found(self, client, mock_get_db_connection):
    ...
    def test_get_supervisions_by_manager(self, client, mock_get_db_connection):
        ...
        mock_conn, mock_cursor = mock_get_db_connection
        mock_cursor.fetchall.return_value = [
            {'supervision_id': 1, 'building': 'Block A',
                'assigned_date': '2024-01-01'},
            {'supervision_id': 2, 'building': 'Block B',
    ...
    def test_get_supervisions_by_building(self, client, mock_get_db_connection):
        ...
        mock_conn, mock_cursor = mock_get_db_connection
        mock_cursor.fetchall.return_value = [
            {'supervision_id': 1, 'personal_id': 'P001', 'assigned_date': '2024-01-01',
             'manager_name': 'John Manager'}
        ]

        response = client.get(
            '/api/building-supervision/by-building/Block%20A')
...
class TestManagerBuildingReport:
    ...
    def test_get_manager_building_report(self, client, mock_get_db_connection):
        ...
        mock_conn, mock_cursor = mock_get_db_connection
        mock_cursor.fetchall.return_value = [
            {'personal_id': 'P001', 'manager_name': 'John Manager', 'building': 'Block A',
             'assigned_date': None, 'maintenance_count': 5, 'chemical_maintenance_count': 1}
        ]
