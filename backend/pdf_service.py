...
class ReportGenerator:
    ...
    def _create_header(self, report_title):
        ...
        elements.append(HRFlowable(
            width="100%", thickness=2, color=POLYU_RED,
    ...
    def _create_kpi_table(self, kpis):
        ...
        for kpi in kpis:
            values_row.append(
                Paragraph(str(kpi['value']), self.styles['KPIValue']))
    ...
    def _create_bar_chart(self, data, x_key, y_key, title, xlabel, ylabel):
        ...
        x_values = [str(item.get(x_key, ''))[:20]
                    for item in data]  # Truncate long labels
    ...
    def _create_pie_chart(self, data, label_key, value_key, title):
        ...
        # PolyU color palette
        chart_colors = ['#A6192E', '#B08E55', '#8C1526', '#5D5D5D',
                        '#A0A0A0', '#D4D4D4', '#E8D4A0', '#6B3A3A']
    ...
    def generate_comprehensive_report(self, report_data, sections=None):
        ...
        """
        if sections is None:
            sections = ['executive_summary', 'maintenance', 'personnel',
    ...
    def _build_executive_summary(self, report_data):
        ...
        kpis = [
            {'label': 'Total Personnel',
                'value': summary.get('total_persons', 0)},
            {'label': 'Departments', 'value': summary.get('total_schools', 0)},
            {'label': 'Activities', 'value': summary.get(
                'total_activities', 0)},
    ...
    def _build_maintenance_section(self, report_data):
        ...
        elements.append(Paragraph("Maintenance Analysis",
                        self.styles['SectionHeader']))
        ...
        chart = self._create_bar_chart(
            maintenance_data, 'building', 'count',
        ...
        if maintenance_data:
            ...
            ] for item in maintenance_data[:15]]  # Limit to 15 rows

            elements.extend(self._create_data_table(
                headers, rows, "Maintenance Summary Table"
            ))
    ...
    def _build_personnel_section(self, report_data):
        ...
        elements = []
        elements.append(PageBreak())
        elements.append(Paragraph("Personnel Overview",
                        self.styles['SectionHeader']))
        ...
        if people_data:
            ...
            elements.extend(self._create_data_table(
                headers, rows, "Personnel by Role and Status"
            ))
    ...
    def _build_activities_section(self, report_data):
        ...
        elements = []
        elements.append(PageBreak())
        elements.append(Paragraph("Activities Statistics",
                        self.styles['SectionHeader']))
        ...
        # Bar chart
        chart = self._create_bar_chart(
            activities_data, 'type', 'activity_count',
            'Activities by Type',
            'Activity Type', 'Count'
        )
        if chart:
        ...
        if activities_data:
            ...
            elements.extend(self._create_data_table(
                headers, rows, "Activities Summary"
            ))
    ...
    def _build_schools_section(self, report_data):
        ...
        elements = []
        elements.append(PageBreak())
        elements.append(Paragraph("Department Statistics",
                        self.styles['SectionHeader']))
        ...
        chart = self._create_bar_chart(
            schools_data, 'school_name', 'affiliated_people',
            'Affiliated Personnel by Department',
            'Department', 'Personnel Count'
        )
        if chart:
            elements.append(chart)
            elements.append(Spacer(1, 20))
        ...
        # Data table
        if schools_data:
            headers = ['Department', 'School Name',
                       'Faculty', 'Personnel', 'Locations']
            rows = [[
            ...
            elements.extend(self._create_data_table(
                headers, rows, "Department Overview"
            ))
    ...
    def _build_safety_section(self, report_data):
        ...
        elements = []
        elements.append(PageBreak())
        elements.append(Paragraph("Safety & Chemical Hazard Report",
                                  self.styles['SectionHeader']))
        ...
        # Summary text
        hazard_count = sum(
            1 for item in safety_data if item.get('active_chemical'))
        elements.append(Paragraph(
        ...
        if freq_data:
            ...
            elements.extend(self._create_data_table(
                headers, rows, "Maintenance Frequency Analysis"
            ))
